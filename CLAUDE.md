...
- シンプルに実装する
- 新規ファイル作成は慎重に判断する
- planモードのときは質問を遠慮なくしてください（最大5回まで）

## 設計指針

実装前に設計を提案すること。以下に従う:
- 複数案を出し、各案のメリット・デメリットを併記
- シンプルでメンテナンス性の高い案を1つ選んで提案
- 選んだ案が破綻する将来の変更パターンも提示

## MCP利用

- symbol検索時は推測せず、必ずget_symbols_overviewで正しいsymbol名を取得すること
